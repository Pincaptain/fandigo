$(document).ready(function(){
    ...
    /************* SOUND SECTION *************/
      ...
      if(!AudioContext) {
        ...
        + ' Please, consider downloading the latest browser version');
    ...
    // You can play the game !!!!
        ...
        function gameStart(){
          ...
          stopGoodTones();
          stopErrTone();
          $('.countnum').text('--').removeClass('color-numoff');
          flashMessage('--',1);
          gameStatus.init();
          addStep();
        }
    
        //Speed of Timer: as a player increases count successes
        function setTimeStep(num){
              var tSteps = [650, 550, 450, 350 ];
              if (num < 4)
                return tSteps[0];
              if (num < 8)
                return tSteps[1];
            ...
            //Flash Start of Game and Error Function
            function flashMessage(msg,times){
              $('.countnum').text(msg);
              var lf = function(){
                $('.countnum').addClass('color-numoff');
                gameStatus.toHndlFl = setTimeout(function(){
                  $('.countnum').removeClass('color-numoff');
                },250);
              };
              var cnt = 0;
              lf();
            ...
            //Display the Number on the Count Clock
            function displayCount(){
              var p = (gameStatus.count < 10) ? '0' : '';
              $('.countnum').text(p+(gameStatus.count+''));
            }
            
            // Play Sequence of a Game
            ...
            // Add another level or step to the game
            function addStep(){
            ...
            // Press Down and Up on the Color Tabs
     ...
     /******** COMPLETE PART 2 ****************/
        ...
        //Switch Button OFF and ON
        $('.offon-btns').on('click', function() {
        $('#power-on').toggleClass('switch-on');
        
         if($('#power-on').hasClass('switch-on') === false){ 
            gameStatus.reset();
            $('.countnum').text('--');
            $('.countnum').addClass('color-numoff');
